...
const game = {
  gameInit: (_io) => {
    ...
    world.on("player_bullet_collistion_event", function (killedPlayer, bullet) {
      entityCreator.createExplosion(
        world,
        killedPlayer.position,
        250,
        bullet.destroyed.fired_by_player_uuid
      );
      entityCreator.createOrReplacePlayerAtCalculatedPosition(
        world,
        killedPlayer.details.name,
        killedPlayer
      );
    });

    world.on(
      "player_blaster_collistion_event",
      function (killedPlayer, blaster) {
        entityCreator.createExplosion(
          world,
          killedPlayer.position,
          250,
          blaster.details.blastOriginatorPlayerUuid
        );
        entityCreator.createOrReplacePlayerAtCalculatedPosition(
          world,
          killedPlayer.details.name,
  ...
  fire: (data)=>{
    ...
    if (userFiringBullet){
      ...
      if (userFiringBullet.cooldown.value == userFiringBullet.cooldown.max){
        ...
        io.emit('action', {
          message: `${userFiringBullet.details.name} Fired! ${power} power @ ${angle}˚`
        })
